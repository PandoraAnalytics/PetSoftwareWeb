...
{
    public partial class managebreedstype : PageBase
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                int id = objCRM.AddBreedType(collection);
                success = (id > 0);
            }
            else
            {
                success = objCRM.UpdateBreedType(collection, this.hdfilter.Value);
            }

            if (success)
            {
                Response.Redirect("managebreedstype.aspx");
            }
            else
            {
