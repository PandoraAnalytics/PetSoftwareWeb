...
{
    public partial class bubreedadd : ERPBase
    {
        ...
        {
            NameValueCollection custcollection = BUCustomer.GetCustomerDetail(ViewState["id"], this.CompanyId);
            if (custcollection != null) ViewState["userid"] = custcollection["userid"];
        ...
        protected void btnAddBreed_Click(object sender, EventArgs e)
        {
            ...
            collection.Add("userid", this.ConvertToString(ViewState["userid"]));

            int breedId = objBreed.AddBreed(collection);
            if (breedId > 0)
            {
                Response.Redirect("bucustomerlanding.aspx?cid=" + this.EncCustomerId);
            }
            else
            {
