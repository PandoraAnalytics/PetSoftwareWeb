...
{
    public partial class ticketlist : PageBase
    {
        ...
        private void PopulateControls()
        {
            ...
            DataSet dataSet2 = Ticket.GetStatuses();
            if (dataSet2 != null && dataSet2.Tables.Count > 0)
            {
                this.ddlStatus.DataSource = dataSet2.Tables[0];
            ...
            if (collection != null)
            {
                ...
                if (status != null)
                {
                    ...
                    {
                        listItem.Selected = (statusArray != null && Array.IndexOf(statusArray, listItem.Value) > -1) ? true : false;
                    }
                }
                this.hdsort.Value = collection["hdsort"];//working
            }
        ...
        private void ApplyFilter()
        {
            ...
            if (status.Length > 0) collection.Add("status", status);
            ...
            objTicket = null;
            collection["hdsort"] = this.hdsort.Value;//working
            Session["ticketsearch"] = collection;

            this.lblEDTSum.Text = "Sum of EDT " + Ticket.GetEDTSum(filter) + " hrs";
        }
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            int totalPages = Ticket.GetTicketsCount(filter);
            if (totalPages > 35)
            {
            ...
            do
            {
                itemDs = Ticket.GetTickets(pageno, filter, this.hdsort.Value);//working
            ...
            string filename = "ticketlist" + BusinessBase.Now.ToString("dd_MM_yyyy_HH_mm") + ".xls";
            try
            {
